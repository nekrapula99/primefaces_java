...
public class SeMpreci00 extends Control implements Serializable{
	...
	private InputText inputTextIdptciColumn; 
	private InputText inputTextIdcmciColumn;  
	private InputText inputTextIdmociColumn;  
...
//			//Validacion lista Compania
		   ...
		   for (Mpreci00 mpreci : listaMpreci) 
		   {
				   if(mpreci00.getIdprci()!=mpreci.getIdprci())
				   {
					   if(mpreci00.getCminci()>mpreci.getCminci() && mpreci00.getCminci()<mpreci.getCmaxci())
					   {
						   ...
						   RequestContext.getCurrentInstance().update(":formDetalle:pnlMpreci00"); 
						   validacionRango=false;
						   break; 
					   }
	...
	private boolean validarDatosRequeridos(){
		...
		if(Utils.isEmptyOrNull(mpreci00.getMpropr00()) && permisoCampos.get("idptci").getReqcam().equals("S"))
		{
			UtilsJSF.facesLog(FacesMessage.SEVERITY_ERROR, getIdiomasHm().get("idptci")+" "+getSeControlFactura().MENSAJES.get("Cam_Obliga"));
			inputTextIdptciColumn.setStyle("border-color: #d2524f;");				
		    RequestContext.getCurrentInstance().update(inputTextIdptciColumn.getClientId());
			validacion = true;
		}else{
			inputTextIdptciColumn.setStyle("border-color: #9a9a9a;");			 
			RequestContext.getCurrentInstance().update(inputTextIdptciColumn.getClientId());	
		}
		
		if(Utils.isEmptyOrNull(mpreci00.getMconca00()) && permisoCampos.get("idcmci").getReqcam().equals("S"))
		{
			UtilsJSF.facesLog(FacesMessage.SEVERITY_ERROR, getIdiomasHm().get("idcmci")+" "+getSeControlFactura().MENSAJES.get("Cam_Obliga"));
			inputTextIdcmciColumn.setStyle("border-color: #d2524f;");				
		    RequestContext.getCurrentInstance().update(inputTextIdcmciColumn.getClientId());
			validacion = true;
		}else{
			inputTextIdcmciColumn.setStyle("border-color: #9a9a9a;");			 
			RequestContext.getCurrentInstance().update(inputTextIdcmciColumn.getClientId());	
		}
		if(Utils.isEmptyOrNull(mpreci00.getMgenus00()) && permisoCampos.get("idmoci").getReqcam().equals("S"))
		{
			UtilsJSF.facesLog(FacesMessage.SEVERITY_ERROR, getIdiomasHm().get("idmoci")+" "+getSeControlFactura().MENSAJES.get("Cam_Obliga"));
			inputTextIdmociColumn.setStyle("border-color: #d2524f;");				
		    RequestContext.getCurrentInstance().update(inputTextIdmociColumn.getClientId());
			validacion = true;
		}else{
			inputTextIdmociColumn.setStyle("border-color: #9a9a9a;");			 
			RequestContext.getCurrentInstance().update(inputTextIdmociColumn.getClientId());	
		}
	...
	public void save() {
		try {										
			...
			if(mpreci00.getCmaxci()>=mpreci00.getCminci()){
				 validarRango();
				 if(!validacionRango)
				    {
				      return;
				 ...
				 UtilsJSF.facesLog(FacesMessage.SEVERITY_ERROR, "La cantidad máxima debe ser mayor o igual a la cantidad mínima");
				 RequestContext.getCurrentInstance().update(":formDetalle:pnlMpreci00");
				 RequestContext.getCurrentInstance().update("formTable");
				 return;			   
			  }
	...
	public InputText getInputTextIdptciColumn() {
		return inputTextIdptciColumn;
	}
	public void setInputTextIdptciColumn(InputText inputTextIdptciColumn) {
		this.inputTextIdptciColumn = inputTextIdptciColumn;
	}
	public InputText getInputTextIdcmciColumn() {
		return inputTextIdcmciColumn;
	}
	public void setInputTextIdcmciColumn(InputText inputTextIdcmciColumn) {
		this.inputTextIdcmciColumn = inputTextIdcmciColumn;
	}
	public InputText getInputTextIdmociColumn() {
		return inputTextIdmociColumn;
	}
	public void setInputTextIdmociColumn(InputText inputTextIdmociColumn) {
		this.inputTextIdmociColumn = inputTextIdmociColumn;
	}
