...
public class SeControlFactura extends Setcontrol implements Serializable
{
	...
	IMgencg00Service mgencg00Service;
		...
		try {
			...
			if (listCompanias == null) {
				...
				for (Musuco00VO comp : listCompanias) {
					if(comp.getIncdco().equalsIgnoreCase("S")){
	...
	public static enum NombresListas{
		...
		listMgencg00Evento, listMcotes00,listMgenus00MonedaCot,listMgenus00UnidadMedidaCot,listTqdpqp00Aux,listMgenteCodtte, listMgenusBarrioSector
	}
		...
		try {
			...
			PERMISOSACCION = (HashMap<String, Object>)session.getAttribute("PERMISOSACCION"); 
					...
					for (Maccgr00 maccgr00Aux : listMaccgr00s) {
						...
						mgencgAux.getMtipre00().setTipore(maccgr00Aux.getId().getTracac());
						mgencgAux.setCodicg(maccgr00Aux.getId().getCoacac());
						mgencgAux= mgencg00Service.Mgencg00ByCodicgEv(mgencgAux.getCodicg(), mgencgAux.getMtipre00().getTipore());
						PERMISOSACCION.put(mgencgAux.getComacg(),mgencgAux.getComacg());
					}
