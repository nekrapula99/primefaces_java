 ...
 /*public void activarBotones(){
		 ...
		 try {
			 ...
			 List<Date> listFechasTqesqe00= new ArrayList<Date>();
				...
				if(!estadoExist){
					...
					if(!flagEstadoInicial){
						 UtilsJSF.facesLog(FacesMessage.SEVERITY_ERROR, "No hay un estado valido para esta cotización");
	...
	public boolean validarFechas(){
		...
		if(tqdrqr00.getFepeqr().after(tqdrqr00.getTqigqg00().getFepdqg())){
			UtilsJSF.facesLog(FacesMessage.SEVERITY_ERROR, "Fecha prometida de entrega debe ser menor o igual a la fecha Promesa Delivery de la cotización");
			RequestContext.getCurrentInstance().update(":formDetalle"); 
	...
	public void save(ActionEvent event) {
		try {
			...
			UtilsJSF.facesLog(FacesMessage.SEVERITY_ERROR, "No se ha creado una cotización para este requerimiento");
			RequestContext.getCurrentInstance().update("formDetalle:tabView:dTableTqdrqr00"); 
			return;	
	...
	//VALIDAR QUE la cotización no este  en estado aprobada o facturada.
